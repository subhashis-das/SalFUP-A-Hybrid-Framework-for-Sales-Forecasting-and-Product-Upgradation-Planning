"""
Decline review analysis using parallel spaCy

Usage:
    python scripts/review_decline_analysis.py <monthly_grouped.csv> <raw_reviews.csv>
"""
...
def normalize_complaint(phrase):
...
def extract_subjects(texts):
    ...
    docs = nlp.pipe(
        texts,
        batch_size=64,
        n_process=max(1, multiprocessing.cpu_count() - 50)
    )
        ...
        for tok in doc:
            if tok.pos_ == "NOUN":
                ...
                if (
                    ...
                    continue
                found = normalize_complaint(extract_phrase(tok))
                break
...
# ---------------- Main ----------------
if len(sys.argv) < 3:
    print("Usage: python review_decline_analysis.py monthly.csv raw.csv")
    sys.exit(1)
...
decline_months = set(monthly.loc[monthly["sales_diff"] < 0, "month"])

raw["month"] = raw["timestamp"].dt.to_period("M").dt.to_timestamp()
decline_reviews = raw[raw["month"].isin(decline_months)]

neg = decline_reviews[
    (decline_reviews["review_text_sentiment"] <= NEGATIVE_THRESHOLD) |
    (decline_reviews["review_title_sentiment"] <= NEGATIVE_THRESHOLD)
]

print(f"Processing {len(neg)} negative reviews...")

texts = [clean_text(t) for t in tqdm(neg["review_text"], desc="Cleaning text")]
subjects = extract_subjects(texts)
subjects = [s for s in subjects if s is not None]
complaint_df = pd.DataFrame({
    "month": neg["month"].values[:len(subjects)],
    "complaint": subjects
})

ranked = (
    complaint_df.groupby(["month", "complaint"])
    .size()
    .reset_index(name="frequency")
    .sort_values(["month", "frequency"], ascending=[True, False])
)

# ---------------- Spell correction ONLY on top complaints ----------------
top = ranked.groupby("month").head(TOP_N_PER_MONTH).copy()
spell = SpellChecker()

# Filter out None or empty complaints
top = top[top["complaint"].notna() & (top["complaint"].str.strip() != "")]
unique = top["complaint"].unique()
...
# Apply spell correction with tqdm
correction_map = {c: correct_phrase(c) for c in tqdm(unique, desc="Spell-correcting")}
top["complaint_corrected"] = top["complaint"].map(correction_map)

# ---------------- Fuzzy merge ----------------
def canonicalize(text):
    return re.sub(r"\s+", " ", text.lower()).strip()

top["complaint_norm"] = top["complaint_corrected"].map(canonicalize)
labels = top["complaint_norm"].unique()
...
top["complaint_root"] = top["complaint_norm"].map(cluster_map)

final_ranked = (
    top.groupby(["month", "complaint_root"])["frequency"]
    .sum()
    .reset_index()
    .sort_values(["month", "frequency"], ascending=[True, False])
)
...
ranked.to_csv("results/complaint_ranking_by_month.csv", index=False)
final_ranked.to_csv("results/complaint_ranking_by_month_root_causes.csv", index=False)
pd.DataFrame(subjects, columns=["subject"]).to_csv(
    "results/negative_subjects_wordnet.csv", index=False
)

print("Saved CSV outputs.")

# ---------------- Wordcloud ----------------
wc_text = " ".join(final_ranked["complaint_root"].tolist())
...
if wc_text:
    ...
    plt.axis("off")
    plt.title("Root Complaint Topics During Decline Months")
    plt.savefig("results/complaint_wordcloud_root_causes.png", dpi=200)
    plt.close()
